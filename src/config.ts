...
export const INSTANCE_CONFIG_TEMPLATE = `# Generated by \`kintio wecom setup\` as <home>/wecom/.env with mode 0600.
# Relative Agent paths resolve from <home>/wecom; shared data stays in <home>/data.
PORT=8888
# WeChat KF credentials. iLink configuration is independent; runtime state is shared.
...
# Shared shutdown and database settings belong in <home>/.env.
...
export interface SharedRuntimeConfig extends IlinkRuntimeConfig {
  readonly home: string;
}
...
export function createConfig(
  ...
  environment = copyEnvironment(environment);
  const home = path.resolve(root || environment.KINTIO_HOME || path.join(os.homedir(), '.kintio'));
  const instanceRoot = path.join(home, 'wecom');
  ...
  const { databaseFile, lockFile } = resolveStateFiles(environment, home);
...
function loadConfigurationEnvironment(options: ConfigLoadOptions, channel: 'wecom' | 'ilink' = 'ilink') {
  ...
  const initialRoot = path.resolve(
    options.root ||
      ...
      (configuredEnvFile
        ? path.resolve(path.dirname(configuredEnvFile), ...(channel === 'wecom' && path.basename(path.dirname(configuredEnvFile)) === 'wecom' ? ['..'] : []))
        : defaultRoot),
  );
  const envFile = path.resolve(
    configuredEnvFile || path.join(initialRoot, ...(channel === 'wecom' ? ['wecom'] : []), '.env'),
  );
...
export function loadSharedRuntimeConfig(options: ConfigLoadOptions = {}): SharedRuntimeConfig {
  const { environment, root } = loadConfigurationEnvironment(options);
  return Object.freeze({ ...createIlinkRuntimeConfig(environment, root), home: root });
}
...
export function loadIlinkRuntimeConfig(
  ...
  const { environment, root } = loadConfigurationEnvironment(options);
  return createIlinkRuntimeConfig(environment, root);
}

function createIlinkRuntimeConfig(environment: NodeJS.ProcessEnv, root: string): IlinkRuntimeConfig {
