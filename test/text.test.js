...
import { splitUtf8, splitWecomText, truncateUtf8 } from '../src/lib/text.js';
...
test('WeCom replies use no more than five 2048-byte messages', () => {
  const chunks = splitWecomText('你'.repeat(5000));

  assert.equal(chunks.length, 5);
  assert.ok(chunks.every((chunk) => Buffer.byteLength(chunk) <= 2048));
  assert.match(chunks.at(-1), /已截断/);
});
